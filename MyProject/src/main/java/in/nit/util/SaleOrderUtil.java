...
public class SaleOrderUtil {
	public void generatePie(String path, List<Object[]>data) {
		...
		JFreeChart chart = ChartFactory.createPieChart3D("SaleOrder  Mode", dataset,true,true,false);
		//3.save as Image using chartutils
		try {
			ChartUtils.saveChartAsJPEG(new File(path +"/resources/images/SaleOrderA.jpg"), chart, 400, 400);
		}catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBar(String path,List<Object[]>data) {
		...
		JFreeChart chart =ChartFactory.createBarChart("SaleOrder Mode", "Modes", "COUNT", dataset);
		//3.save as Image using chartutil
		try {
			ChartUtils.saveChartAsJPEG(new File(path +"/resources/images/SaleOrderB.jpg"), chart, 400, 400);
		}catch (IOException e) {
			e.printStackTrace();
		}
	}
