...
public class PurchaseOrderUtil {
	public void generatePie(String path, List<Object[]>data) {
		...
		JFreeChart chart = ChartFactory.createPieChart3D("PurchaseOrder  Mode", dataset,true,true,false);
		//3.save as Image using chartutils
		try {
			ChartUtils.saveChartAsJPEG(new File(path +"/resources/images/PurchaseOrderA.jpg"), chart, 400, 400);
		}catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBar(String path,List<Object[]>data) {
		...
		JFreeChart chart =ChartFactory.createBarChart("PurchaseOrder Mode", "Modes", "COUNT", dataset);
		//3.save as Image using chartutil
		try {
			ChartUtils.saveChartAsJPEG(new File(path +"/resources/images/PurchaseOrderB.jpg"), chart, 400, 400);
		}catch (IOException e) {
			e.printStackTrace();
		}
	}
